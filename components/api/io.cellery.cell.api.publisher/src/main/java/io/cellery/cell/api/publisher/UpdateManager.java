...
public class UpdateManager {
        ...
        for (API api : apis) {
            if (api.isGlobal()) {
                ...
                if (existingApiId.equals(Constants.Utils.EMPTY_STRING)) {
                    ...
                    ApiUpdateRequest globalApiUpdatePayload = createGlobalApiUpdatePayload(api, newApiVersionId);
  ...
  private static String updateGlobalAPI(ApiUpdateRequest globalApiUpdatePayload, String id)
      throws APIException {
        ...
        try {
            apimConfig = ConfigManager.getAPIMConfiguration();
            apiUpdatePath = restConfig.getApimBaseUrl() + Constants.Utils.PATH_PUBLISHER
                    + restConfig.getApiVersion() + Constants.Utils.PATH_APIS + "/" + id;
            apiUpdateResponse = requestProcessor
                    .doPut(apiUpdatePath, Constants.Utils.CONTENT_TYPE_APPLICATION_JSON,
                            Constants.Utils.CONTENT_TYPE_APPLICATION_JSON,
                            Constants.Utils.BEARER + apimConfig.getApiToken(),
    ...
    private static String getExistingApiId(API api) throws APIException {
        ...
        String apiRetrieveQuery = Constants.Utils.CELLNAME_N_CONTEXT_PROPERTY + ":" + (cellConfig.getCell() +
                getContext(api)).replace("/", "_");
        String apiRetrievePath = restConfig.getApimBaseUrl() + Constants.Utils.PATH_PUBLISHER
                + restConfig.getApiVersion() + Constants.Utils.PATH_QUERY + apiRetrieveQuery;
    ...
    private static Boolean abelToCreateNewVersion(API api) throws APIException {
        ...
        String apiRetrieveQuery = Constants.Utils.CELLNAME_N_CONTEXT_PROPERTY + ":" + (cellConfig.getCell() +
                getContext(api)).replace("/", "_") + " " +
                Constants.JsonParamNames.VERSION + ":" + getVersion(api);
        String apiRetrievePath = null;
        try {
            apiRetrievePath = restConfig.getApimBaseUrl() + Constants.Utils.PATH_PUBLISHER
                    + restConfig.getApiVersion() + Constants.Utils.PATH_QUERY
